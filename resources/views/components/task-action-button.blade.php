...
@elseif($tasks->status === "completed")
    @if(auth()->user()->hasRole(['super admin','admin','account manager']))
        Status: <span class="text-success text-bold">Completed</span>, <i>Do you want to </i><button type="button" class="btn btn-xs btn-warning" value="{{$tasks->id}}" data-toggle="modal" data-target="#set-status">Re-open?</button>
